...
async function informes() {
    ...
    if (!informesGuardados) {
        ...
        informesGuardados.informesGuardados.forEach(informes => {
            informe1 = informes[i].split('|')
            anio = informe1[0]
            tipoRecuento = informe1[1]
            tipoEleccion = informe1[2]
            categoriaId = informe1[3]
            distrito = informe1[4]
            seccionProvincial = informe1[5]
            seccionID = informe1[6]
...
async function cambiarTitulo() {
    ...
    titulo.innerText = `Elecciones <br>${anio} |<br>${tipoEleccion == 2 ? "Generales" : "Paso"}`;
}
...
async function cambiarSubtitulo() {
    ...
    subtitulo.innerText = `${ anio } ><br> ${tipoEleccion == 2 ? "Generales" : "Paso"} ><br> Definitivo ><br>${ cargo } ><br> ${ distrito } ><br> ${ seccion }`
}

async function cambioImagen() {
    var distrito = distritosSelect.value
    var imgMap = document.getElementById("mapa")
